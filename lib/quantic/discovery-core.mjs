...
const DISCOVERY_HANDLE_NAMESPACE = "quantic-handle:";
...
export function normalizeDiscoveryHandle(value) {
  ...
  if (!HANDLE.test(handle) || handle.includes("~")) {
    throw new Error("Handle Quantic court de découverte invalide.");
  }
  return handle;
}
...
export function discoveryHandleKey(value) {
  const handle = normalizeDiscoveryHandle(value);
  return createHash("sha256").update(`${DISCOVERY_HANDLE_NAMESPACE}${handle}`).digest("hex");
}
...
function validateOptionalCryptoProfile(profile, identityManifest, pinnedProfile, routeManifest, options) {
  ...
  if (!profile) {
    ...
    if (pinnedProfile?.payload?.policy === "hybrid-required") {

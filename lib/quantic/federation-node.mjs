...
import { assertEnvelopeMeetsCryptoPolicy } from "./pqc-policy.mjs";
...
export function verifyPortableEnvelope(
  ...
  const canonicalText = canonicalPortableEnvelopeText(value);
  if (!verifyP256(device.deviceSigningPublicKey, canonicalText, value.signatures.p256Device)) {
    throw new Error("Signature P-256 de l’enveloppe Quantic invalide.");
  }

  let verifiedProfile = null;
  if (senderCryptoProfile) {
    verifiedProfile = verifyCryptoProfile(senderCryptoProfile, manifest, null);
  }
  const policy = assertEnvelopeMeetsCryptoPolicy(value, verifiedProfile);

  if (policy.pqDevice) {
    const signature = value.signatures.mlDsa65Device;
    if (
      !signature ||
      !mlDsaVerify(
        policy.pqDevice.mlDsaPublicKeySpki,
        Buffer.from(canonicalText, "utf8"),
        signature,
      )
    ) {
      throw new Error("Signature ML-DSA-65 de l’enveloppe Quantic invalide.");
    }
  }
